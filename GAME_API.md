...
- When one alias is `null` and the other is numeric but invalid (for example fractional, low/negative, or otherwise out-of-range), response remains `400 INVALID_FIELD` because null/type validation runs first.
